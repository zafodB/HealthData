...
def extract_query(json_contents: dict):
    if json_contents['replyCount'] < 2:
        return None

    query = {'category': json_contents['commonCategory']}
    if 'annotationsFull' not in json_contents['replies'][0]:
        query['annotations'] = []
    else:
        query['annotations'] = json_contents['replies'][0]['annotationsFull']
...
def make_queries(start_dir: str) -> list:
    ...
    for directory in subdirs:
        for root, dirs, files in os.walk(os.path.join(starting_directory, directory)):
            ...
            for filename in files:
                ...
                new_query = extract_query(contents)
...
def calculate_relevance_scores(query: dict, json_contents: dict):
    scored_documents = {}
        ...
        if index == 0:
            ...
            try:
                ...
                if 'annotationsFull' in reply:
                    number_medical_entities = len(reply['annotationsFull'])
                    number_same_entities = 0
                    for entity in reply['annotationsFull']:
...
def loop_all_documents(queries: list, start_dir: str):
        ...
        for filename in files:
            ...
            if contents['replyCount'] < 2:
                ...
                for query in queries:
                    ...
                    relevance_scores = calculate_relevance_scores(query, contents)
...
queries = make_queries(starting_directory)
...
loop_all_documents(queries, starting_directory)
